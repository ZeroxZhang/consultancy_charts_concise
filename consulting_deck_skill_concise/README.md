...
`qa_deck` 的三档与逐页失效半径见[验收](references/qa.md)与[交付契约](references/delivery.md)：迭代与冒烟档用于制作期，`package_delivery` 只接受验收档的 audit。
