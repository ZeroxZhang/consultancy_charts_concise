...
function page(title,lead,body,source=src,tracker='出海研究 · 设计迭代样稿',arch='D-04',status='材料沿用 · 待事实复核'){
 const n=slides.length+1;slides.push(`<section class="slide reading${n===1?' active':''}" data-arch="${arch}" data-frame-boundary="integrated">${frame.markup}<div class="slide__tracker">${tracker}</div><div class="slide__sticker">${status}</div><header class="slide__header"><h1 class="slide__title">${title}</h1><div class="slide__lead">${lead}</div></header><div class="slide__body">${body}</div><div class="source">${source}</div><div class="slide__page">${n}</div></section>`);
}
...
['中国A股','15%','上市样本、地域与收入分母'],['欧洲','54%','国家/公司覆盖与汇总方法'],['日本','51%','企业样本与海外业务定义'],['美国','40%','行业结构、分母与统计范围']],[23,18,59]),'原稿数值只作为待核记录；样本构成、期间和收入定义一致后才建立可比较基线。')}<div class="exhibit"><h2>02｜企业层面需要的证据</h2>${notes([['需求与可进入市场','明确可服务客户、准入条件与竞争基准。'],['贡献与资本占用','核对价格、全成本、现金周转和实施投入。'],['目标形成方式','从企业能力与可获得增量构建目标，不用跨市场百分比差直接外推。']])}</div></div>`,src,'原稿P04 → 比较资格与证据需求','D-05');
...
['规模','同范围市场口径、期间、币种和可服务边界','可比数值或区间；样本分组','先核实范围'],['增速','相同时间窗口与基数，实际/预测区别','趋势、分布与异常解释','不由增速直接排优先'],['壁垒','客户、技术、渠道、合规的具体机制','有依据的能力/机制比较','不凭主观1–5分定强弱'],['政策风险','适用市场、情景、传导机制与暴露','条件分支与应对措施','高低档需可观察锚点']],[12,35,29,24])}</div><div class="layout-three">${notes([['保留框架价值','用共同维度组织尽调，丰富材料可以继续扩展分析。'],['避免伪精确','原稿无评分定义与证据，暂不使用色阶、面积或综合分。'],['获得证据后再比较','补齐锚点、权重和敏感性，或直接用经济性与约束作取舍。']])}</div>`,src+' 原稿评分仅作为历史输入，不作为本页分析结果。','原稿P17 → 定性证据框架','D-06','评价依据 · 待验证');
