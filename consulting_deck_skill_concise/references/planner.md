...
选型之前先跑一次遍历：`node scripts/sweep_forms.cjs`（`--family <族>` 只看一族的候选，`--json` 给机器读，`--check` 只做一致性核对）。它把**本技能真的能画出来的形式**按分析族列全，并标出 planner 目录里哪些能力本技能没有实现入口——**送进 planner 但本技能画不出来的能力，只会变成一张写不出成稿的规格**。

混合材料、复杂比较、图表与表格取舍、机制与数字组合，或现有表达无法承担本页证明任务时，需要实质选型。

**直接制作**只适用于三种情况：该页 `form` 已经写进 `pages.json`、纯查数与文字编辑、复用已验收的成熟页面。**"我心里已经想好画什么"不算"已明确图型"**——先写出 `form`；写不出，或写出来发现它只是这一族里最省事的那个，就是需要实质选型的信号。不为走流程调用 planner，也不为遵循流程增加固定代理。

反过来，**同一形式在本稿已经用到第 3 次**时，下一页不能再默认沿用它：要么调用 planner 看这一族里还有没有更合适的入口，要么在 `pages.json` 写 `repetitionReason`。这不是要求换图，而是要求这一页的选型是一次真实判断。
...
**本机安装的版本要跟得上随包快照。** 装旧了或装残了（例如缺 `capabilities.json`）会被判为 `incompatible`，加载器虽然会继续回退，但每次都要多试一轮，而且你读到的 `SKILL.md` 可能是旧版。用 `node scripts/sync_installed_planner.cjs --dry-run` 先看会改哪些文件，确认后去掉 `--dry-run` 执行：它只写快照里有的文件（不动目标目录里的 `tests/` 等其他内容），写完立刻按 lock 复验；`--target` 可指定别的安装位置。

缓存默认写在 `$HOME/.cache/consulting-deck-skill-concise`；**该位置不可写时（沙箱、容器、受限 $HOME）自动回退到技能根目录的 `.cache/`，再回退到系统临时目录**，"默认命令必然失败"不再等于跳过选型。`--cache-dir` 是显式指定：给了就只用它，出错如实报错，不再静默回退。
