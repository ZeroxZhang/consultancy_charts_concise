...
**形式名后的 `†` 表示该形式已接入通用标注层，可以直接声明 `annotations` 写旁解读。** 没有 `†` 的形式（`kit.tree`、`kit.swimlane`、`kit.processFlow`、`kit.comparisonTable`、全部 `diagram.*`、全部 `html.*`、`svg.custom`）**不能**声明 `annotations`，`check_pages.cjs` 会直接拒绝；要贴引线标注时改用带 `†` 的形式，或把解释写进正文与图注。
...
- **取舍**：**必须共用同一量尺**，否则图形会夸大差异；带面积填充时**基准线要画在填充之上**，否则基期那条线被填充吃掉，六张图就没有共同参照了。刻度只在首列出现，每张只标自己的变化量，绝对值交给公共刻度——六张各标一套绝对值等于六套坐标。用 `svg.custom` 时 `visual` 必填，成稿要带同名 `data-visual`。每格只有 384 宽，图内标签实际落在 12–13px，这是这一条的固有代价。
