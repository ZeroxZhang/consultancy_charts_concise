...
![version](https://img.shields.io/badge/version-v8.0-2251FF)
![claude-code](https://img.shields.io/badge/Claude%20Code-Agent%20Skill-D97757)
![deliverable](https://img.shields.io/badge/deliverable-HTML%20%2B%20PDF-E34F26)
...
`consulting_deck_skill` is a Claude Code Agent Skill: give it a brief and your raw materials, and it runs the full **analysis → storyline → visual design → build → independent QA** pipeline, delivering a **self-contained HTML deck and a matching paginated PDF**. The HTML supports 16:9 or 4:3, paging, zoom, presentation mode, deep links, and offline one-click download of the validated PDF.
...
| PPTX requires Office; fonts and layouts break everywhere | HTML opens in a browser; the matching PDF is ready to share and archive |
...
- 📦 **HTML + PDF delivery** — the standalone PDF is ready to archive; the HTML embeds the same PDF for one-click offline download and retains browser printing.
...
open iteration_v8_delivery/delivery/consulting-deck-v8-reference.html # v8 dual-format delivery sample
```
...
- **v7** — unified typography roles, pinned font resources, offline subsets, real glyph measurement and font QA
- **v8** (current) — HTML + PDF delivery, offline one-click download, and delivery-version consistency gates

See the `iteration_v*/` archives for details.

## Disclaimer

The three themes are independent adaptations based on **public visual materials**, not official internal templates of any consulting firm. This repository delivers HTML and paginated PDF, and does not claim native editable PPTX output.
