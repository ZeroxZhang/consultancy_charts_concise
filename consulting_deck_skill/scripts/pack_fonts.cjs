...
function pack(html,{profile,extraText='',assetDir}={}){
  ...
  const specs=[...html.matchAll(/data-(?:spec|opt)=(['"])([\s\S]*?)\1/g)].map(m=>decode(m[2])).join('');
  if(p.faces.length)bundle=JSON.parse(execFileSync(process.env.FONT_PYTHON||'python3',[path.join(__dirname,'font_assets.py')],{input:JSON.stringify({faces:p.faces,text,assetDir,html,supportText:collect('',extraText+specs),roleFamilies:{title:families(p.title),body:families(p.body)}}),encoding:'utf8',maxBuffer:30*1024*1024}));
