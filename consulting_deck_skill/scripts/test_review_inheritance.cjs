...
function fixture(name){
 ...
 const create=(folder,changed)=>{
  ...
  for(let page=1;page<=2;page++)for(const medium of ['html','pdf']){const file=path.join(folder,medium+'-'+page+'.png');fs.writeFileSync(file,'synthetic '+medium+' screenshot page '+page+(page===2?changed:''));entries.push({id:medium+':p'+page,medium,page,pageId:'p'+page,path:path.basename(file),sha256:fileHash(file),pageSha256:hash('page '+page+(page===2?changed:'')),sourceSha256:artifacts[medium+'Artifact'].sha256,dependenciesSha256:deps});}
  return {pages:2,geometryStatus:'PASS',errors:[],documentContract:{reliability:'2',kind:'fragment'},taskContract:normalize({kind:'fragment',complexity:'simple'}),...artifacts,evidenceManifest:{version:1,dependenciesSha256:deps,entries}};
 };
 const coverage=pages=>({reviewer:'author',independence:'author',layers:[...layers],htmlPages:pages,pdfPages:pages,evidence:pages.flatMap(page=>['html','pdf'].map(m=>({id:m+':p'+page})))});
 const review=(audit,rows)=>({schemaVersion:3,status:'complete',reviewer:'author',independence:'author',htmlSha256:audit.htmlArtifact.sha256,pdfSha256:audit.pdfArtifact.sha256,auditSha256:hash(stable(audit)),checks:Object.fromEntries(['analysis','evidence','visual'].map(key=>[key,{status:'pass',basis:'仅合成结构测试 '+key}])),issues:[],coverage:rows});
