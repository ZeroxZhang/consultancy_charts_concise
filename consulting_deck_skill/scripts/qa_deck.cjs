...
(async()=>{
 ...
 const report={input,pages:n,pdfPages,navigation:{fullscreen,deepLink,scales},errors,offline:offlineState,rows,visualStatus:'NOT_REVIEWED：必须实际查看每页图片与PDF',geometryStatus:rows.some(r=>r.overflow.length||r.smallDataText.length||r.charts.some(c=>!c.rendered||c.error))||errors.length?'FAIL':'PASS'};
