...
function render(payload){
  ...
  if(payload.recipe)plan=runtime.prepare(payload.recipe,payload.spec||{},settings);
    ...
    plan={width:settings.width,height:settings.height,reason:null,pages:[{kind:'chart',option:runtime.options(payload.option,settings.tokens,settings.fontSize,profile.id)}]};
  ...
  return {...plan,typography_id:profile.id,typography_version:profile.version,measurement,font_delivery:'requires-embedding-in-host',pages:plan.pages.map((page,index)=>{
    const chart=echarts.init(null,null,{renderer:'svg',ssr:true,width:settings.width,height:settings.height});
    try{chart.setOption(page.option);if(runtime.check(chart,plan,index)!==plan)throw Error('配方未完成回退');const svg=chart.renderToSVGString().replace('<svg ',`<svg data-typography="${profile.id}" style="font-synthesis:none;text-rendering:geometricPrecision;font-variant-numeric:lining-nums tabular-nums" `);if(/(?:translate|matrix|[MLCQ])[^<>]*\b(?:NaN|Infinity)\b/.test(svg))throw Error('SVG出现非有限坐标');return {...page,svg};}finally{chart.dispose();}
  })};
}
