...
这是本项目的排版方案，不是咨询公司的官方字体规范。字体版本独立于配色3.0.0、分析模式和ECharts6.1.0。
...
仅封面、章节、页主判断使用衬线，不能将所有h2改为衬线。重要限制按解释性正文呈现，不压进12px Source；不要默认细字重、中文斜体或拉大字距。强调使用真实600而非全页粗体。

DM Serif Text仅提供400常规/斜体，本项目使用常规。引擎对主标题西文片段标记.type-latin使用400，中文600；font-synthesis:none防伪粗。标题中的日期/百分比保留标题字体，正文数字自然排列。数据表和图表文字启用lining-nums/tabular-nums，HTML、浏览器测量探针和SSR fontkit使用同一数字特性。混排西文优先回退中文家族，中文标点保留；页面语言需正确标记。

## 配置与继承

- serif-report：研究报告默认，Noto Serif SC600 + DM Serif Text400标题；Noto Sans SC / Inter正文。
...
源字体随skill提供，不需再下载。只有重建源资源才运行 `$FONT_PYTHON scripts/font_assets.py prepare`，再运行 `node scripts/sync_typography.cjs`；审核资源清单和版本后重建成稿，不手改生成的字体CSS/引擎配置快照。
...
ExhibitKit、ChartRuntime和render_echarts_svg输入均接受typography_id；浏览器先加载deck-typography.js。HTML比较表继承所在报告配置。SSR使用fontkit + 固定资源 + tnum测字宽；400/500/600分别匹配真实字重。ChartRuntime统一fontFamily及font简写，未提供的斜体会报错。容量预检仍含保守估算，不能替代最终浏览器字形边界验收。
