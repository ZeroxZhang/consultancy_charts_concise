...
**每页必须显式声明边界**：`data-frame-boundary` 是页面契约的一部分，不留静默默认值。制作时逐页写入 line / integrated / space（封面与全出血页 `data-frame="off"`）；不写就按 line 渲染的静默行为会造成「标题线 + 正文首排顶线」双层边界或线贴内容的观感问题，属于视觉缺陷而非母版选项。
...
2. **边条内边距**：页面级注释/判断条（`.annotation` 及同类块）左侧 3px 边条与文字之间必须保留内边距（组件自带 12px；行内重写 border/background 时不得省略 padding-left），文字贴边条属于视觉缺陷。
3. **首排顶线合并**：正文首排模块已有顶线（如 `.exhibit h2` 的 brand 顶线、`.kpi-card`、全宽表格上边）时用 integrated 省去标题线；出现「灰线紧贴正文顶线」的双线即边界选择或 margin/offset 配对错误。
