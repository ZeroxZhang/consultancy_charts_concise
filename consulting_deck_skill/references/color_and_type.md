...
实体较多或跨页复用时，视觉规范可提供实体登记表：`实体 | token | 可用图型 | 标签 | 例外`。
...
字体按typography_system.md：固定一套中西文主标题搭配与一套无衬线阅读/数据搭配，默认最多四个实际家族；来源和数字复用正文家族。统一配置为assets/deck-typography.js，与配色独立。数据右对齐且lining-nums/tabular-nums，解释性口径14–15px，不压为12px来源。
标题通常两行，按实际测量换行，不强行用20中文字截断完整判断。
