...
禁用装饰侧边条判断块及顶部色条数字卡，包括自定义类、伪元素和SVG同类外观；数据条、轴线、关系线、必要结构分隔和quiet母版除外。关键限制贴近对象，少量关键否定/分母/状态/条件按delivery_system.md绑定；保证同层文字基线、共享比较轨道、实际绘图区及来源安全区，不能只齐外框。
...
```text
请独立验收{deck绝对路径}，输出{目录}/review.independent.json。
输入：{原始用户目标、已明确偏好与认可/否定基线、task合同、已有分析与原材料定位、作者规格、HTML、当前audit、逐页截图与实际PDF}。
先读{skill}/references/delivery_system.md的现行schemaVersion 3格式，再读workflow_qa.md、page_frame.md、analysis_review.md、color_and_type.md，
...
母版与组件合规必查（workflow_qa.md 的 E-V01…04）：每页 boundary 显式声明、标题灰线与正文必要分隔线不重复、无装饰侧边条及顶部色条卡片（包含自定义类、伪元素和SVG变体）、页边识别不被遮挡；先按 page_frame.md「检查与交付」核对。语义数据条、轴线、必要分隔与quiet母版不误杀。
逐页核对模块内空置、全页剩余空间、同层文字基线、比较轨道、真实绘图区及来源安全区。外框相齐不代表内容对齐；允许有效非对称和有目的留白，不接受短文字大空框或机械等高填充。已确认缺陷按用户明确要求返工，重看完整HTML/PDF页。
...
按delivery_system.md返回可直接聚合的review JSON：记录真实独立性、检查范围、具体依据、页号/严重级/影响/修复建议及未验收项，coverage与checks引用当前audit的产物、页面身份与实际证据。不要仅返回qa_report.md，也不重复创造另一套字段。未看到的图片不能登记看图通过；HTML截图不能当PDF证据。
关键否定、分母、状态和条件需对照原文与对象，不能用总体文字覆盖率替代。局部修订仅在无变更页身份及原证据关联核对后继承检查；有影响页面重新看图，不能替换旧截图哈希冒认新检查。
...
独立审查按task的complexity/majorConclusion风险决定，kind只决定首尾；缺省保守按complex，简单任务明确登记simple。普通局部编辑可核对后继承未受影响检查。S7实际看图后返回delivery_system.md的同结构review JSON，绑定当前产物、audit与页面证据；不得由脚本伪造目视结论。方法、图型、布局和建议字号可自主调整，不因未填写候选表/编号/评分退回；客观错误、关键遗漏、禁用色条、整页关系/重心或有效对齐缺陷及未兑现本次明确成品要求均需返工。
